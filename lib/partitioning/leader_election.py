...
class LeaderElection(AbstractPartitioningMethod):
    ...
    def _partition_impl(self, problem):
        ...
        start = time.time()
        seed_nodes = np.random.choice(G.nodes,
                                      self.num_partitions,
                                      replace=False)
        ...
        while np.sum(partition_vector == -1) != 0:
            ...
            new_node = np.random.choice(
                np.argwhere(partition_vector == -1).flatten())
            ...
            neighboring_partitions = np.unique([
                partition_vector[x] for x in nx.all_neighbors(G, new_node)
            ...
            while len(neighboring_partitions) == 0:
                ...
                new_node = np.random.choice(
                    np.setdiff1d(np.argwhere(partition_vector == -1).flatten(), already_tried))

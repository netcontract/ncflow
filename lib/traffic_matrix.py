...
class TrafficMatrix(object):
    ...
    @property
    def is_full(self):
        return np.alltrue(np.argwhere(self._tm == 0.0) == np.array([[i, i] for i in range(self.tm.shape[0])]))

    def serialize(self, dir_path, fmt='pickle'):
        if fmt == 'pickle':
            with open(
                    os.path.join(dir_path,
                                 '{}_traffic-matrix.pkl'.format(self._fname)),
                    'wb') as w:
                pickle.dump(self.tm, w)
        elif fmt == 'text':
            np.savetxt('{}_traffic-matrix.txt'.format(self._fname),
                       self.tm,
                       fmt='%10.7f',
                       delimiter=' ')
        else:
    ...
    def from_file(cls, fname):
        ...
        if model == 'gravity':
            ...
            return GravityTrafficMatrix(problem=None, tm=tm, total_demand=None,
                                        random=random, seed=seed, scale_factor=scale_factor)
        elif model == 'uniform':
            return UniformTrafficMatrix(problem=None, tm=tm, max_demand=None,
                                        seed=seed, scale_factor=scale_factor)
        elif model == 'real':
            date, time = vals[0], vals[1]
            return RealTrafficMatrix(problem=None, tm=tm, date=date, time=time,
                                     seed=seed, scale_factor=scale_factor)
        elif model == 'bimodal':
            fraction = float(vals[0])
            low_range = tuple([float(x) for x in vals[1].split('-')])
            high_range = tuple([float(x) for x in vals[2].split('-')])
            return BimodalTrafficMatrix(problem=None, tm=tm, fraction=fraction,
                                        low_range=low_range, high_range=high_range, seed=seed, scale_factor=scale_factor)
        elif model == 'behnaz':
            ...
            return PoissonTrafficMatrix(problem=None, tm=tm, lam=lam, decay=decay,
                                        const_factor=const_factor, seed=seed, scale_factor=scale_factor)
        elif model == 'exponential':
            decay, const_factor = float(vals[0]), float(vals[1])
            return ExponentialTrafficMatrix(problem=None, tm=tm, decay=decay,
                                            const_factor=const_factor, seed=seed, scale_factor=scale_factor)
    ...
    @property
    def _fname(self):
        return '{}_{}_{}_{}_{}'.format(self.problem.name, self.model, self.seed, self.scale_factor, self._fname_suffix)
    ...
    def _init_traffic_matrix(self):
        raise NotImplementedError(
            '_init_traffic_matrix needs to be implemented in the subclass: {}'.
...
class GravityTrafficMatrix(TrafficMatrix):
    ...
    def copy(self):
        return GravityTrafficMatrix(self.problem, self._tm.copy(), self.total_demand, self.random, self.seed, self.scale_factor)
...
class UniformTrafficMatrix(TrafficMatrix):
    ...
    def copy(self):
        return UniformTrafficMatrix(self.problem, self._tm.copy(), self.max_demand, self.seed, self.scale_factor)
...
class ExponentialTrafficMatrix(TrafficMatrix):
    ...
    def copy(self):
        return ExponentialTrafficMatrix(self.problem, self._tm.copy(), self.beta, self.decay, self.const_factor, self.seed, self.scale_factor)
    ...
    def _init_traffic_matrix(self):
        ...
        self._tm = np.array([[np.random.exponential(self._beta * (self._decay**dist)) for dist in row] for row in distances],
                            dtype=np.float32)
...
class PoissonTrafficMatrix(TrafficMatrix):
    ...
    def copy(self):
        return PoissonTrafficMatrix(self.problem, self._tm.copy(), self.lam, self._decay, self._const_factor, self.seed, self.scale_factor)
    ...
    def _init_traffic_matrix(self):
        ...
        self._tm = np.array([[np.random.poisson(self._lam * (self._decay**dist)) for dist in row] for row in distances],
                            dtype=np.float32)
        # No traffic between node and itself
        np.fill_diagonal(self._tm, 0.0)
...
class GaussianTrafficMatrix(TrafficMatrix):
    ...
    def copy(self):
        return GaussianTrafficMatrix(self.problem, self._tm.copy(), self.mean, self.stddev, self.seed, self.scale_factor)
...
class BimodalTrafficMatrix(TrafficMatrix):
    ...
    def copy(self):
        return BimodalTrafficMatrix(self.problem, self._tm.copy(), self.fraction, self.low_range, self.high_range, self.seed, self.scale_factor)
    ...
    def _init_traffic_matrix(self):
        ...
        inds = np.random.choice(2, (num_nodes, num_nodes), p=[
                                self.fraction, 1 - self.fraction]).astype('bool')
    ...
    @property
    def _fname_suffix(self):
        return '{}_{}-{}_{}-{}'.format(self.fraction, self.low_range[0], self.low_range[1], self.high_range[0], self.high_range[1])
...
class RealTrafficMatrix(TrafficMatrix):
    ...
    def copy(self):
        return RealTrafficMatrix(self.problem, self._tm.copy(), self.date, self.time, self.seed, self.scale_factor)
    ...
    def _update(self, scale_factor, type, **kwargs):
        if type == 'uniform':
            ...
            raise Exception(
                '"{}" not a valid perturbation type for the traffic matrix'.
                format(type))

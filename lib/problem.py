...
class Problem(object):
    def __init__(self,
        ...
        if traffic_matrix is not None:
            if isinstance(traffic_matrix, np.ndarray):
                ...
                self.traffic_matrix = GenericTrafficMatrix(self, traffic_matrix, scale_factor=scale_factor)
            ...
            raise Exception(
                'no traffic matrix passed in; model argument "{}" is invalid'.
                format(model))
    ...
    def from_file(cls, topology_fname, traffic_matrix_fname, old_way=True):
        if topology_fname.endswith('-edgelist.txt'):
            G = nx.read_edgelist(topology_fname,
    ...
    def multi_commodity_list(self):
        if not hasattr(self, '_multi_commodity_list'):
            self._multi_commodity_list = [
                (k, [x], [y], z)
                for k, (x, y,
                        z) in enumerate(commodity_gen(self.traffic_matrix.tm))
            ]
    ...
    def sparse_commodity_list(self):
        if not hasattr(self, '_sparse_commodity_list'):
            self._sparse_commodity_list = list(
                enumerate(
                    commodity_gen(self.traffic_matrix.tm, skip_zero=False)))
    ...
    def new_capacities(self,
        ...
        self._change_capacities(min_cap=min_cap,
                                max_cap=max_cap,
    ...
    def is_traffic_matrix_full(self):
        return len(self.commodity_list) == self.traffic_matrix.tm.size - self.traffic_matrix.tm.shape[0]

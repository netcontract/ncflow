...
from ...graph_utils import compute_residual_problem, path_to_edge_list, assert_flow_conservation
...
class NCFlowEdgePerIter(AbstractFormulation):
    ...
    @classmethod
    def new_max_flow(cls, num_paths, edge_disjoint=True, dist_metric='inv-cap', out=None, **args):
    ...
    def __init__(self, *, objective, num_paths, edge_disjoint, dist_metric, DEBUG, VERBOSE, out=None, **args):
    ...
    def divide_problem_into_partitions(self, problem, partition_vector):
        ...
        for partition_id in np.unique(partition_vector):
            ...
            G_meta_no_edges.add_node(
                ...
                pos=np.mean([
                    ...
                    if orig_G.nodes.data('pos')[n] else (0.0, 0.0)
                    for n in nodes
                ],
                    axis=0))
        ...
        if self.VERBOSE:
            ...
            self._print('\nmeta -> virtual: {}'.format(
                pretty_print_str(meta_to_virt_dict)))
        ...
        self.subgraphs = [G_subgraph for part_id,
                          G_subgraph in sorted(subgraph_dict.items())]
    ...
    def init_data_structures(self):
        ...
        self.r2_G_hats = [[G_subgraph.copy() for G_subgraph in self.subgraphs] for _ in range(self.max_num_iters)]
        ...
        self.all_u_hat_ins = [[set() for _ in self.G_metas[0].nodes()] for _ in range(self.max_num_iters)]
        self.all_v_hat_outs = [[set() for _ in self.G_metas[0].nodes()] for _ in range(self.max_num_iters)]
        ...
        for meta_node_id, r2_G_hat in enumerate(self.r2_G_hats[iter]):
            for u, v in r2_G_hat.edges():
                ...
                if u in self.virt_to_meta_dict:
                    orig_u = self.selected_inter_edges[iter][(self.virt_to_meta_dict[u], meta_node_id)][0]
                if v in self.virt_to_meta_dict:
                    orig_v = self.selected_inter_edges[iter][(meta_node_id, self.virt_to_meta_dict[v])][1]
                r2_G_hat[u][v]['capacity'] = residual_problem.G[orig_u][orig_v]['capacity']
    ...
    def select_inter_edges(self):
        ...
        for (u_meta, v_meta), inter_edge_list_no_cap in self.meta_edge_dict.items():
            ...
            inter_edge_list = [(u, v, orig_G[u][v]['capacity']) for u, v in inter_edge_list_no_cap]
            # sort from highest capacity to lowest capacity
            highest_cap_inter_edges = sorted(inter_edge_list, key=lambda x: x[-1], reverse=True)[:self.max_num_iters]
    ...
    def hash_partition(self, iter):
        ...
        sorted_inter_edges = sorted([(u_meta, v_meta, u, v) for (u_meta, v_meta), (u, v)
                                     in self.selected_inter_edges[iter].items()])
        inter_edges_str = '--'.join('-'.join(str(x) for x in group)
                                    for group in sorted_inter_edges)
        # raw fname is too long for Unix filesystem, so we compute md5 digest
        return hashlib.md5((partition_vector_str + inter_edges_str).encode('utf-8')).hexdigest()

    ############
    # R1 PATHS #
    ############
    @staticmethod
    def r1_paths_full_fname(problem_name, hash_partition_str, num_paths, edge_disjoint, dist_metric):
        paths_dir = R1_PATHS_DIR.format(problem_name, hash_partition_str)
        if not os.path.exists(paths_dir):
            os.makedirs(paths_dir)
        return os.path.join(paths_dir,
                                  'r1_{}-paths_edge-disjoint-{}_dist-metric-{}-dict.pkl'.format(num_paths,
                                                                                                edge_disjoint,
                                                                                                dist_metric))
    ...
    def compute_r1_paths(self):
        full_fname = NCFlowEdgePerIter.r1_paths_full_fname(self.problem.name, self.hash_partition(0),
                self._num_paths, self.edge_disjoint, self.dist_metric)
    ...
    def get_all_r1_paths(self):
        ...
        from_nodes = set([node for node, degree in self.G_metas[0].out_degree() if degree > 0])
        to_nodes = set([node for node, degree in self.G_metas[0].in_degree() if degree > 0])
        for s_k_meta, t_k_meta in product(from_nodes, to_nodes):
    ...
    ############
    @staticmethod
    def r2_paths_full_fname(meta_node_id, problem_name, hash_partition_str, num_paths, edge_disjoint, dist_metric):
    ...
    def compute_r2_paths_for_meta_node(self, meta_node_id, iter):
        full_fname = NCFlowEdgePerIter.r2_paths_full_fname(meta_node_id,
                self.problem.name, self.hash_partition(iter), self._num_paths,
        ...
        subgraph_nodes = np.argwhere(
            self._partition_vector == meta_node_id).flatten()
            ...
            if s != t:
                try:
                    paths_dict[(s, t)] = find_paths(
                        G_hat_weighted, s, t, self._num_paths, disjoint=self.edge_disjoint)
    ...
    def get_all_r2_paths_for_meta_node(self, meta_node_id, iter):
        ...
        self._print('Loading R2 paths from pickle file, iter {}'.format(iter), full_fname)
        try:
            with open(full_fname, 'rb') as f:
                paths_dict = pickle.load(f)
                for key, paths in paths_dict.items():
        ...
        for s, t in product(from_nodes, to_nodes):
            if s != t:
                if (s, t) not in paths_dict:
                    raise Exception(
                            'No path from {} to {} in r2 paths dict for meta-node {}!'.format(s, t, meta_node_id))
    ...
    def select_r1_paths(self):
        ...
        r1_paths_selected_so_far = {k_meta: set() for k_meta, _ in self.meta_commodity_list}
        for iter in range(self.max_num_iters):
            for k_meta, (s_k_meta, t_k_meta, _) in self.meta_commodity_list:
                all_paths_and_weights = sorted([
                        (path, compute_weight(self.G_metas[0], path, self.dist_metric)) for path in self.r1_paths_full_dict[(s_k_meta, t_k_meta)]
                    ], key=lambda x: x[-1])
                paths_to_choose_from = [
                        path for path, _ in all_paths_and_weights if tuple(path) not in r1_paths_selected_so_far[k_meta]
                    ]
    ...
    def pre_solve(self, problem, partitioner):
        ...
        if len(np.unique(
                self._partition_vector)) != partitioner.num_partitions:
            if self.VERBOSE:
                self._print(
                    '{} partitions requested, but {} partitions generated'.
                    format(partitioner.num_partitions,
                           len(np.unique(self._partition_vector))))
                for i, part_id in enumerate(np.unique(self._partition_vector)):
                    if i == part_id:
                        continue
                    self._partition_vector[np.argwhere(
                        self._partition_vector == part_id).flatten()] = i
        self._print(self._partition_vector)
        self._save_txt(self._partition_vector,
        ...
        for meta_node_id in np.unique(self._partition_vector):
            for iter in range(self.max_num_iters):
                self.r2_paths_dicts[iter].append(self.get_all_r2_paths_for_meta_node(meta_node_id, iter))


    def solve(self, problem, partitioner):
        def get_ncflow_obj(iter):
            if self.out.name == 'stdout' or self.out.name == '<stdout>':
                return NcfSi(objective=self._objective, DEBUG=self.DEBUG,
                            VERBOSE=self.VERBOSE, out=self.out, **self._args)
            else:
                log = open(self.out.name.replace(
                    '.txt', '_iter_{}.txt'.format(iter)), 'w')
                return NcfSi(objective=self._objective, DEBUG=self.DEBUG,
                            VERBOSE=self.VERBOSE, out=log, **self._args)
        ...
        for iter in range(self.max_num_iters):
            ...
            self._print(
                'Computing residual problem after iteration {}'.format(iter))
            ...
            if flow_before_current_iter > 0.0 and nc.obj_val / flow_before_current_iter < 0.05:
                self._print(
    ...
    def sol_dict(self):
        if not hasattr(self, '_sol_dict'):
            ...
            # universal mapping
            src_target_pair_to_commod = {(commod[1][0], commod[1][1]): commod for commod in
    ...
    def check_feasibility(self):
        ...
        for min_u, min_v, min_cap in bottleneck_edges[:20]:
            ...
            self._print('({}, {}), residual capacity: {}, {}'.format(
    ...
    def fib_entries(cls, problem, partitioner, num_paths, edge_disjoint, dist_metric):
        ...
        ncflow = cls.new_max_flow(num_paths=num_paths, edge_disjoint=edge_disjoint, dist_metric=dist_metric)
    ...
    def num_fib_entries_for_path_set(self):
        ...
        for (k_meta, (s_k_meta, t_k_meta, _)), orig_commod_list_in_k_meta in self.meta_commodity_dict.items():
            meta_commod_id_str = 'k-meta-{}'.format(k)
            subgraph_src_nodes = np.argwhere(self._partition_vector == s_k_meta).flatten()
            subgraph_target_nodes = np.argwhere(self._partition_vector == t_k_meta).flatten()
            ...
            for iter in range(self.max_num_iters):
                ...
                for src in subgraph_src_nodes:
                    ...
                    if r1_path_id not in self.fib_dict[src][meta_commod_id_str]:
                        self.fib_dict[src][meta_commod_id_str][r1_path_id] = set(r2_path_ids)
                    else:
                        for r2_path_id in r2_path_ids:
                            self.fib_dict[src][meta_commod_id_str][r1_path_id].add(r2_path_id)
                ...
                for u_meta, v_meta in path_to_edge_list(r1_path[1:]):
                    ...
                    for r2_path_id, r2_path in zip(r2_path_ids, r2_paths):
                        ...
                        if meta_commod_id_str not in self.fib_dict[ingress_transit_node]:
                            self.fib_dict[ingress_transit_node][meta_commod_id_str] = {}
                        if r1_path_id not in self.fib_dict[ingress_transit_node][meta_commod_id_str]:
                            self.fib_dict[ingress_transit_node][meta_commod_id_str][r1_path_id] = set()

                        self.fib_dict[ingress_transit_node][meta_commod_id_str][r1_path_id].add(r2_path_id)
                    ...
                    virtual_src = self.meta_to_virt_dict[u_meta][0] # advance virtual in node for next iteration

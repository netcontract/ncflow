...
class AbstractTest(object):
    ...
    def assert_feasibility(self, ncflow):
        try:
            ...
            self.has_error = True
            print(bcolors.ERROR + '[ERROR] NCFlow did not find feasible flow' + bcolors.ENDC)
    ...
    def assert_eq_epsilon(self, actual_val, correct_val, epsilon=1e-5):
        try:
            ...
            self.has_error = True
            print(bcolors.ERROR + '[ERROR] Correct value: {}, actual value: {}'.format(
                correct_val, actual_val) + bcolors.ENDC)
    ...
    def assert_geq_epsilon(self, actual_val, lower_val, epsilon=1e-5):
        try:
            ...
            self.has_error = True
            print(bcolors.ERROR +
                  '[ERROR] Looking for >= {}, actual value: {}'.format(lower_val, actual_val) +
                  bcolors.ENDC)
    ...
    def assert_leq_epsilon(self, actual_val, upper_val, epsilon=1e-5):
        try:
            ...
            self.has_error = True
            print(bcolors.ERROR +
                  '[ERROR] Looking for <= {}, actual value: {}'.format(upper_val, actual_val) +
                  bcolors.ENDC)

    def run(self):
        raise NotImplementedError(
            'run needs to be implemented in the subclass: {}'.format(
                self.__class__))

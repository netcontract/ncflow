...
def find_paths(G, s_k, t_k, num_paths, disjoint=True, include_weight=False):
    ...
    def k_shortest_edge_disjoint_paths(G, source, target, k, weight='weight'):
        ...
        return [remove_cycles(path) for path in sorted(nx.edge_disjoint_paths(G, s_k, t_k), key=lambda path: compute_distance(path))[:k]]

    if disjoint:
        if include_weight:
            return [(path, compute_weight(path)) for path in k_shortest_edge_disjoint_paths(G, s_k, t_k, num_paths, weight='weight')]
        else:
            return k_shortest_edge_disjoint_paths(G, s_k, t_k, num_paths, weight='weight')
    else:
        if include_weight:
            return [(path, compute_weight(path)) for path in k_shortest_paths(G, s_k, t_k, num_paths, weight='weight')]
        else:

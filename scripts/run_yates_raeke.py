...
YATES_TOPO_DIR = os.path.abspath(
    os.path.join(TL_DIR, 'topologies', 'yates-format'))
YATES_TM_DIR = os.path.abspath(
    os.path.join(TL_DIR, 'traffic-matrices', 'yates-format'))
...
def run_yates(args):
    ...
    problem_name_dot = problem_name.replace('.graphml',
                                            '.dot').replace('.json', '.dot')
    cmd = [
        'yates',
        os.path.join(YATES_TOPO_DIR, problem_name_dot),
        os.path.join(YATES_TM_DIR, tm_fname),
        os.path.join(YATES_TM_DIR, tm_fname),
        os.path.join(YATES_HOSTS_DIR,
                     problem_name_dot.replace('.dot', '.hosts')), '-raeke',
        '-budget',
        str(num_paths)
    ]
    print(' '.join(cmd))
    ...
    os.chdir(
        os.path.join(YATES_RESULTS_DIR, problem_name_dot.replace('.dot', ''),
                     'paths'))
    os.rename(
        'raeke_0',
        os.path.join(OUTPUT_DIR,
                     '{}-{}-paths-rrt.txt'.format(problem_name, num_paths)))
...
if __name__ == '__main__':
    ...
    for problem_name in PROBLEM_NAMES:
        ...
        for tm_fname in iglob(
                os.path.join(YATES_TM_DIR,
                             '{}_traffic-matrix.txt'.format(problem_name))):

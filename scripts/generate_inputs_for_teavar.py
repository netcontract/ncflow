...
if __name__ == '__main__':
    ...
    for prob_name in ["Uninett2010.graphml", "b4-teavar.json"]:
        ...
        tm_fname = list(
            iglob('../traffic-matrices/{}/{}*_traffic-matrix.pkl'.format(
                TMFOLDERS[0], prob_name)))[0]
        if prob_name.endswith('.graphml'):
            problem = Problem.from_file(
                '../topologies/topology-zoo/{}'.format(prob_name), tm_fname)
        else:
            problem = Problem.from_file('../topologies/{}'.format(prob_name),
                                        tm_fname)
                    ...
                    with open(tm_fname, 'rb') as f:
                        ...
                        for row in full_tm[:-1, :]:
                            print(' '.join(str(x) for x in row),
                                  ...
                                  file=w)
                        print(' '.join(str(x) for x in full_tm[-1]),
                              end='\n',
                              file=w)
                        f.close()
            w.close()
        ...
        for numpaths in [4, 8]:
            path_names = list(
                iglob("../topologies/raecke/{}-{}-paths*".format(
                    prob_name, numpaths)))
            if len(path_names) == 0:
                continue
            if len(path_names) > 1:
                print("WARN... multiple matching path files {}".format(
                    path_names))
        ...
        for numpaths in [4, 8]:
            path_names = list(
                iglob(
                    "../topologies/paths/{}-{}-paths_edge-disjoint-True_dist-metric-inv-cap*.pkl"
                    .format(prob_name, numpaths)))
            if len(path_names) == 0:
                continue
            if len(path_names) > 1:
                print("WARN... multiple matching path files {}".format(

...
if __name__ == '__main__':
    ...
    for prob_name in PROBLEM_NAMES:
        ...
        for node, data in problem.G.nodes.data(True):
            ...
            problem.G.nodes[node]['mac'] = '00:00:00:00:{}:{}'.format(
                node, node)
        ...
        for u, v, cap in problem.G.edges.data('capacity'):
            if cap == 0.0:
                ...
                problem.G[u][v]['capacity'] = '{}Gbps'.format(
                    np.around(cap / 1e3))
        ...
        nx.drawing.nx_agraph.write_dot(
            ...
            os.path.join(
                ...
                prob_name.replace('.graphml', '.dot').replace('.json',
                                                              '.dot')))

        with open(
                os.path.join(
                    HOSTS_OUTPUT_DIR,
                    prob_name.replace('.graphml',
                                      '.hosts').replace('.json', '.hosts')),
                'w') as w:

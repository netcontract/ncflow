...
class PF(object):
    def __init__(self):
        num_paths, edge_disjoint, dist_metric = PATH_FORM_HYPERPARAMS
        self.pf = PathFormulation.new_max_flow(num_paths=num_paths,
                                               edge_disjoint=edge_disjoint,
                                               dist_metric=dist_metric)
...
class NcI(object):
    def __init__(self, G):
        ...
        self.ncflow = NcfEpi.new_max_flow(num_paths=num_paths,
                                          edge_disjoint=edge_disjoint,
                                          dist_metric=dist_metric)
...
if __name__ == '__main__':
    seed_tm_fname = glob('../traffic-matrices/{}/{}_{}_*_{}_*.pkl'.format(
        TM_MODEL, TOPOLOGY, TM_MODEL, SCALE_FACTOR))[0]
    ...
    if sys.argv[-1] == '--delta-norms':
        ...
        demand_tracking(algo, problems, time_per_prob, residual_factor,
                        results_file_placeholder)
